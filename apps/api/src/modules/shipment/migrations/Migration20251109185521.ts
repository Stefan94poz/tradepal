...
export class Migration20251109185521 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "shipment_tracking" ("id" text not null, "order_id" text not null, "vendor_id" text null, "carrier" text not null, "tracking_number" text not null, "status" text check ("status" in ('pending', 'in_transit', 'delivered', 'failed')) not null default 'pending', "current_location" text null, "estimated_delivery" timestamptz null, "last_updated" timestamptz not null, "tracking_events" jsonb not null, "is_parent_order" boolean not null default false, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "shipment_tracking_pkey" primary key ("id"));`);
    ...
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_shipment_tracking_vendor_id" ON "shipment_tracking" ("vendor_id") WHERE deleted_at IS NULL;`);
  }
